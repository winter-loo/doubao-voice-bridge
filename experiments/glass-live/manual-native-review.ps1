...
function SourceCheck {
    ...
    $allowed=@('experiments/glass-live/src/voice_window.rs','experiments/glass-live/src/manual_native_review.rs','experiments/glass-live/manual-native-review.ps1','experiments/glass-live/MANUAL-NATIVE-REVIEW.md')
...
if ($Mode -eq 'Build') {
    ...
    try {
        ...
        if ($code -ne 0 -or [regex]::Matches($listing,('(?m)^'+[regex]::Escape($test)+': test\r?$')).Count -ne 1) { throw 'Exact manual test entry not found' }
        SourceCheck; ProtectedFiles
        $record=[ordered]@{review_commit=$ExpectedCommit;production_source_commit=$base;production_window_body_unchanged=$true;run_directory=$run;executable=$delivery;sha256=(Hash $delivery);test=$test;build_exit_code=0;windows_started=$false;scope='Test harness compiled only; not a new formal product EXE or native-window acceptance'}
        $manifestPath=Join-Path $run 'build-manifest.json'; SaveJson $manifestPath $record
        Write-Host ('[BUILD COMPLETE] review_commit='+$ExpectedCommit+'; production_source='+$base)
    ...
    if ($m.review_commit -cne $ExpectedCommit -or $m.production_source_commit -cne $base -or $m.test -cne $test -or $m.build_exit_code -ne 0) { throw 'Review manifest identity mismatch' }
    ...
    Write-Host '结束正在进行的语音输入。请准备手动点击胶囊、点阴影边缘、拖动后再点阴影；可用顶部按钮切换背景、重播、波形。'
    Write-Host '点击“退出”结束；最长三分钟自动关闭。当前正式客户端不会退出或替换。'
    ...
    Write-Host "`n[USER ACTION REQUIRED] 请评价刚才实际显示：白底暗化自然，最终文字和完整外阴影可见，拖动时主体/阴影一起移动。"
    $visual=Read-Host '全部符合输入 PASS；有问题输入 FAIL；未观察完整输入 UNREVIEWED'
    if ($visual -cnotin @('PASS','FAIL','UNREVIEWED')) { $visual='UNREVIEWED' }
    SaveJson (Join-Path $session 'human-review.json') ([ordered]@{review_commit=$ExpectedCommit;production_source_commit=$base;report='explicit terminal response after visible review';visual=$visual;manual_input_passed=$native.manual_input_passed;deployment_performed=$false})
    Write-Host ('[UPLOAD FILE] '+$reportPath); Write-Host ('[UPLOAD FILE] '+(Join-Path $session 'human-review.json'))
