...
float4 adaptive_reduce_ps(float4 position:SV_POSITION):SV_TARGET {
    ...
    [unroll] for(int j=0;j<3;j++) { [unroll] for(int i=0;i<5;i++) {
        ...
        mean+=wide/15;
        float y=dot(wide,LUMA);second+=y*y/15;
    ...
    // Large flat bright scenes need edge separation, not a permanent gray plate.
    ...
    float4 old_material=image1.Load(int3(1,0,0));
    bool first=old.a<.5;
    ...
    float light_y=light_target*current_y+.008+.012*white_risk;
    float dark_y=dark_target*current_y+.008;
...
float adaptive_text(float2 p) {
    ...
    float sharp=text_mask.SampleLevel(clamped,uv,0);
    float clarity=adaptive_time.z>.5?1:smoothstep(0,.11,adaptive_time.y);
    float radius=geometry.w*.034*(1-clarity);
    float2 step=float2(radius/geometry.z,radius/geometry.w);
    float soft=sharp*.40;
    soft+=(text_mask.SampleLevel(clamped,uv+float2(step.x,0),0)
          +text_mask.SampleLevel(clamped,uv-float2(step.x,0),0)
          +text_mask.SampleLevel(clamped,uv+float2(0,step.y),0)
          +text_mask.SampleLevel(clamped,uv-float2(0,step.y),0))*.15;
...
float4 adaptive_material_ps(float4 pos:SV_POSITION):SV_TARGET {
    ...
    float4 material=adaptation.Load(int3(1,0,0));
    float2 dummy,ds;
    ...
    float3 body=adaptive_transmit(scene,dark,material);
