...
fn production_optimizing_preserves_approved_optical_pixels()->AppResult<()>{unsafe{
    ...
    for dark in [false,true]{for kind in 0..4{
        ...
        for y in 0..rh{for x in 0..rw{
            ...
            };input[((y*rw+x)*4) as usize..][..4].copy_from_slice(&color);
        }}
        for p in [&current,&approved]{p.context.UpdateSubresource(&p.raw.texture,0,None,input.as_ptr().cast(),rw*4,0);p.prepare();}
        for frame in 0..4{
    ...
    eprintln!("[voice-parity] approved 7a3d03c shader vs production optimizing; 32 cases; max RGBA error={max}; synthetic WARP");
}Ok(())}
